package br.unb.cic.epl

import org.scalatest.FlatSpec
import org.scalatest.Matchers


class TestSubUnitV extends FlatSpec with Matchers {


  var literal100: Literal = _
  var literal200: Literal = _

  literal100 = new Literal(100)
  literal200 = new Literal(200)

  var teste = "(100 - 200)"
  var test2 = -100 

  val sub = new Sub(literal100, literal200)

  if(sub.print() == teste){
    print("\n====>> Teste unitario SUB print:  (100 - 200) e igual a " + sub.print() + " ==> ")
    print("O teste passou com sucesso!!\n\n")
  }else{
    print("\n====>> Teste unitario SUB print:  (100 - 200) e diferente de " + sub.print() + " ==> ")
    println("O teste falhou!!\n\n")
  }

  val eval = new Eval()

  sub.accept(eval)

  if(eval.result() == test2){
    print("\n====>> Teste unitario SUB: " + sub.print() + " e igual a " + eval.result() + " ==> ")
    print("O teste passou com sucesso!!\n\n")
  }else{
    print("\n====>> Teste unitario SUB: " + sub.print() + " e igual a " + eval.result() + " mas deveria ser (100 - 200) = -100" + " ==> ")
    println("O teste falhou!!\n\n")
  }
  
}
